...
const ArtisanDashboard = () => {
  ...
  const allArtisanProducts = products.filter(
    (product) => product.artisanId === isSeller._id
  );

  // Filter products by status
  const pendingProducts = allArtisanProducts.filter(
    (product) => product.status === "pending"
  );
  const approvedProducts = allArtisanProducts.filter(
    (product) => product.status === "approved"
  );
  const rejectedProducts = allArtisanProducts.filter(
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "orders":
        return <OrdersTab currency={currency} orders={orders} />;
      default:
        return (
          <OverviewTab
            products={allArtisanProducts}
